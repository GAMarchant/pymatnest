...
.. glossary::
    ...
    ``n_spin_steps=int`` 
       | Ratio of spin-polarised trajectories will be determined by ``n_spin_steps``/SUM(``n_*_steps``).
       | default: 1

    ``spin_traj_len=int`` 
       | Length of spin-polarised trajectory in each spin type step.
       | default: 8

    ``spin_traj_len_cost_multiplier=int`` 
       | Multiplier for cost of a spin-type step (set to 1 for MD, TE-HMC, and SP-MC with O(1) cost moves, N for naive SP-MC)
       | default: 1

    ``break_up_spin_traj=[T | F]`` 
       | Whether to intersperse ``n_spin_steps`` spin-type sub-trajectories with other types of steps.
       | default: F 
    ...
    ``MD_spin_timestep=float`` 
       | default: 0.1 (ASE time units)

    ``MD_spin_timestep_max=float`` 
       | default: 2.0 (ASE time units)

    ``MD_spin_energy_fuzz=float``
       | Tolerance for rejecting non-energy conserving trajectories, as fraction of Kinetic Energy
       | default: 1.0e-2

    ``MD_spin_reject_energy_violation=[ T | F ]``
       | Use energy conservation violation (exceeding MD_atom_energy_fuzz * KE) to reject MD trajectories.
       | default: F
    ...
    ``magnetic=[ T | F ]``
       | Perform simulation with spin-polarised atoms, enabling other spin-related options. Work in progress.
       | default: F

    ``fixed_moments=[ T | F ]``
       | If true, walkers are assigned equivalent magnetic moments and they are not allowed to vary during simulation. Requires magnetic=T. Work in progress.
       | default: F

    ``set_moments=str [fm | afm]``
       | If fm, magnetic moments are set to ferromagnetic (all aligned along +z direction). Requires magnetic=T. Work in progress (anti-ferromagnetic option to be added).
       | default:

    ``B_field=str``
       | Set a constant, uni-directional external magnetic field (format: |B| b_x b_y b_z, b = unit vector of B (units determined by lammps)). Work in progress.
       | default: "0 0 0 1"
    ...
    sys.stderr.write("magnetic=[ T | F ] (F, work in progress, requires energy_calculator=lammps and atom_algorithm=MD. If true, atomic magnetic moments, as well as related options, are enabled.)\n")
    sys.stderr.write("fixed_moments=[ T | F ] (F, requires magnetic=T. If true, moments do not move during MD steps.)\n")
    sys.stderr.write("spin_step_fixed_pos=[ T | F ] (F, work in progress, requires magnetic=T. If true, atomic positions are fixed during MD_spin_step.)\n")
    sys.stderr.write("set_moments=[ T | F ] (F, work in progress, requires magnetic=T. If true, each walker is assigned the same configuration of moments. Currently only works with randomised and ferromagnetic moments (anti-ferromagnetic and file-input are work in progress))\n")
    sys.stderr.write("B_field=str (0 0 0 1, requires magnetic=t. Set an external magnetic field, format: |B| b_x b_y b_z, b = unit vector of B (units determined by lammps)\n")
...
def eval_energy(at, do_KE=True, do_PE=True, do_BM=True):
    ...
    if at.arrays['initial_magmoms'] is None:
        do_BM=False
...
def propagate_lammps(at, dt, n_steps, algo, Emax=None, fixed_moments=False):
    ...
    if algo == 'NVE':
        if ns_args['magnetic']:
            if fixed_moments:
                ...
                if movement_args['spin_step_fixed_pos']:
                    pot.lmp.command('fix 1 all nve/spin lattice frozen') #apply magnetic constant energy fix w/ fixed positions.  
                else:
                    pot.lmp.command('fix 1 all nve/spin lattice moving') #apply magnetic constant energy fix w/ fixed moments.
                #B_field = movement_args['B_field']
                #pot.lmp.command(f"fix B_field all precession/spin zeeman {B_field}")
        else:
        ...
        if algo == 'NVE':
            if ns_args['magnetic']:
                if fixed_moments:
                    ...
                    pot.propagate(at, properties=['energy','forces', 'magmoms', 'magnorm', 'E_mag', 'T_mag'],system_changes=['positions', 'magmoms'], n_steps=n_steps, dt=dt, magnetic_field=movement_args['B_field'])
                #at.info['magnorm'] = pot.results['magnorm']
                #at.info['E_mag'] = pot.results['E_mag']
                #at.info['T_mag'] = pot.results['T_mag']
            else:
...
def perturb_3D_vec(vec, delta_r, delta_theta, delta_phi):
    vec_r = np.linalg.norm(vec)
    vec_theta = np.arccos(vec[2]/vec_r)
    if vec[0] > 0:
        vec_phi = np.arctan(vec[1]/vec[0])
    elif vec[0] < 0:
        vec_phi = np.arctan(vec[1]/vec[0]) + np.pi
    else:
        vec_phi = np.pi/2
    vec_r += rng.normal(delta_r)
    vec_theta += rng.normal(delta_theta)
    vec_phi += rng.normal(delta_phi)
    vec[0] = vec_r*np.cos(vec_phi)*np.sin(vec_theta)
    vec[1] = vec_r*np.sin(vec_phi)*np.sin(vec_theta)
    vec[2] = vec_r*np.cos(vec_theta)
    return vec
...
def eval_mag_norm(at, vector=False):
    mx_mean = np.average(at.arrays['initial_magmoms'][:,0])
    my_mean = np.average(at.arrays['initial_magmoms'][:,1])
    mz_mean = np.average(at.arrays['initial_magmoms'][:,2])
    m_mag = np.linalg.norm(at.arrays['initial_magmoms'][0,:])
    magnorm = np.linalg.norm(np.array([mx_mean, my_mean, mz_mean]))/m_mag
    if vector:
        return np.array([mx_mean, my_mean, mz_mean])
    else:
        return magnorm
...
def magmoms_sd_angle(at):
    ...
    magmoms_mean = eval_mag_norm(at, vector=True)
    mean_size = np.linalg.norm(magmoms_mean)
    angle_sq_sum = 0
    ...
    angle_sd = np.sqrt(angle_sq_sum/len(at))
...
def spin_swap_flip(at, mmind_1='random', mmind_2='random', swap='random', flip_type='random', max_perturb=np.pi, MC_ratio=0.5, calc_energy=False, potts_param=0):
    ...
    if mmind_2 == 'random':
        mmind_2 = mmind_1 - rng.int_uniform(1, len(at.arrays['initial_magmoms']))

    if swap == 'random':
        swap = rng.int_uniform(0, 2)

    if flip_type == 'random':
        flip_type = rng.int_uniform(0, 7)
    ...
    m1 = at.arrays['initial_magmoms'][mmind_1]

    m2 = at.arrays['initial_magmoms'][mmind_2]
    ...
    if flip_type == 1:
        ...
        at.arrays['initial_magmoms'][mmind_1] = np.cross(at.arrays['initial_magmoms'][mmind_1], at.arrays['initial_magmoms'][mmind_2])
    ...
    if swap == 1:
        at.arrays['initial_magmoms'][[mmind_1, mmind_2]] = at.arrays['initial_magmoms'][[mmind_2, mmind_1]]

    if calc_energy:
        energy = eval_energy(at)
        return energy

def do_MD_spin_walk(at, movement_args, Emax, KEmax):
#DOC
#DOC ``do_MD_spin_walk``
    ...
    #print(print_prefix, "LIVIA 4A", at.info['ns_energy'], eval_energy(at))
    pre_MD_E = at.info['ns_energy']
    #print(print_prefix, "LIVIA 4B", at.info['ns_energy'], eval_energy(at), pre_MD_E)
    ...
    if movement_args['python_MD']:
        ...
        final_E = None
        timestep = movement_args['MD_spin_timestep']
        ...
        if do_calc_quip:
            propagate_NVE_quippy(at, dt=movement_args['MD_spin_timestep'], n_steps=movement_args['spin_traj_len'])
            final_E = eval_energy(at)
        elif do_calc_lammps:
            #print(print_prefix, "LIVIA 4C - pre-propagate", at.info['ns_energy'], eval_energy(at), eval_energy(at, do_PE=False))
            if propagate_lammps(at, dt=movement_args['MD_spin_timestep'], n_steps=movement_args['spin_traj_len'], algo='NVE', fixed_moments=False):
                final_E = eval_energy(at) # pot.results['energy'] + eval_energy(at, do_PE=False)
                #final_E = spin_swap_flip(at, mmind_1='random', mmind_2=0, swap=0, flip_type=7, max_perturb=movement_args['MD_spin_timestep'], calc_energy=True) # eval_energy(at, do_PE=True)
                #####
                #results_energy = pot.results['energy']
                #eval_energy_pe = eval_energy_PE(at)
                #poteng = at.get_potential_energy()
                #toteng = at.get_total_energy()
                #kineng = eval_energy_KE(at)
                #mageng = eval_energy_ME(at)
                #diff = results_energy - poteng
                #magnorm = at.info['magnorm']
                #header = print_prefix, "LIVIA 4C - post-propagate results[energy], eval_energy_pe, poteng, kineng, mageng, toteng, mageng+toteng, pot_diff, magnorm"
                #init_magmoments = print_prefix, "LIVIA 4C - post-propagate", at.get_initial_magnetic_moments()
                #magmoments = print_prefix, "LIVIA 4C - post-propagate", at.get_magnetic_moments()
                #data = print_prefix, "LIVIA 4C - post-propagate", results_energy, eval_energy_PE(at), poteng, kineng, mageng, toteng, mageng+toteng, '{:.10f}'.format(diff), magnorm
                #print(header) 
                #pot.lmp.command(f"# {header}")
                #print(data)
                #pot.lmp.command(f"# {data}")
                #print(init_magmoments)
                #print(magmoments)
                #pot.lmp.command(f"# {init_magmoments}")
                #pot.lmp.command(f"# {magmoments}")
            else: # propagate returned success == False
                final_E = 2.0*abs(Emax)
            #print(print_prefix, "LIVIA 4C - final", at.info['ns_energy'], eval_energy(at), final_E)
        elif do_calc_fortran:
            final_E = f_MC_MD.MD_atom_NVE_walk(at, n_steps=movement_args['spin_traj_len'], timestep=movement_args['MD_spin_timestep'], debug=ns_args['debug'])
    ...
    if abs(final_E-pre_MD_E) > movement_args['MD_spin_energy_fuzz']*final_KE:
        if movement_args['MD_spin_reject_energy_violation']:
            reject_fuzz = True
        # else:
    ...
    # print(print_prefix, "LIVIA 4D", at.info['ns_energy'], eval_energy(at), final_E)
    #DOC \item if reject
    ...
    #DOC \item else
        ...
        if movement_args['MD_atom_velo_flip_accept']:
            at.set_velocities(-at.get_velocities())
        if movement_args['MD_spin_flip_trials'] > 0:
            pre_flip_moms = at.get_initial_magnetic_moments()
            for i in range(movement_args['MD_spin_flip_trials']):
                #E_flip = 2.0*abs(Emax) #GEORGE PLACEHOLDER WHILE TESTING
                E_flip = spin_swap_flip(at, mmind_1='random', mmind_2='random', swap='random', max_perturb=magmoms_sd_angle(at), calc_energy=True)
                #E_flip = spin_swap_flip(at, mmind_1='random', mmind_2='random', swap='random', max_perturb=np.pi, calc_energy=True)
                if E_flip <= Emax:
                    final_E = E_flip
                    break
                else:
    ...
    #    print("LIVIA 4D0 - post perturb?")
    #    do_MC_atom_velo_walk(at, movement_args, Emax, nD, KEmax)

    # print(print_prefix, "LIVIA 4E", at.info['ns_energy'], eval_energy(at), final_E, n_accept)
    return {'MD_spin' : (1, n_accept) }
...
#DOC ``do_MD_atom_walk``
    ...
    if movement_args['python_MD']:
        ...
        if do_calc_quip:
            ...
            if propagate_lammps(at, dt=movement_args['MD_atom_timestep'], n_steps=movement_args['atom_traj_len'], algo='NVE', fixed_moments=True):
                final_E = eval_energy_PE(at) + eval_energy(at, do_PE=False) 
                #final_E = pot.results['energy'] + eval_energy(at, do_PE=False)
                #print(print_prefix, "LIVIA 4C - post-propagate", pot.results['energy'], at.get_potential_energy(), float(pot.results['energy'] - at.get_potential_energy()))
            else: # propagate returned success == False
                final_E = 2.0*abs(Emax)
        elif do_calc_fortran:
...
def do_MC_spin_walk(at, movement_args, Emax):

    n_steps = movement_args['spin_traj_len']
    step_size = movement_args['MC_spin_step_size']
    n_try = n_steps
    n_accept=0
    n_accept_velo = None
    ...
    for i_MC_step in range(n_steps):
        ...
        if energy >= Emax:
            # reject move
            at.set_initial_magnetic_moments(orig_moms)
            if ns_args['n_extra_data'] > 0:
...
def do_spin_walk(at, movement_args, Emax, KEmax):
#DOC
#DOC ``do_atom_walk``
    n_reps = movement_args['n_spin_steps_per_call']
    out={}
    #DOC \item loop n\_atom\_steps\_per\_call times, calling do\_MC\_atom\_walk() or do\_MD\_atom\_walk()
    for i in range(n_reps):
        if movement_args['spin_algorithm'] == 'MC':
            accumulate_stats(out, do_MC_spin_walk(at, movement_args, Emax))
        elif movement_args['spin_algorithm'] == 'MD':
            accumulate_stats(out, do_MD_spin_walk(at, movement_args, Emax, KEmax))
        else:
            exit_error("do_spin_walk got unknown 'spin_algorithm' = '%s'\n" % movement_args['spin_algorithm'], 5)

    return (n_reps*movement_args['spin_traj_len']*movement_args['spin_traj_len_cost_multiplier'], out)
...
#DOC ``walk_single_walker``
    ...
    if movement_args['do_good_load_balance']:
        ...
        nums = np.array( [ movement_args['n_atom_steps_n_calls'],
                           movement_args['n_spin_steps_n_calls'],
        ...
        costs = np.array( [ movement_args['atom_traj_len']*movement_args['atom_traj_len_cost_multiplier'],
                            movement_args['spin_traj_len']*movement_args['spin_traj_len_cost_multiplier'],
                            1,
                            1,
                            1,
                            1,
        ...
        possible_moves = ( [do_atom_walk] * movement_args['n_atom_steps_n_calls'] +
                           [do_spin_walk] * movement_args['n_spin_steps_n_calls'] +
...
def max_energy(walkers, n, kinetic_only=False, magnetic=False):
...
#DOC ``full_auto_set_stepsizes``
    ...
    if (comm is not None):
        ...
        key_flags =  1*np.asarray([ i in key_ints for i in range(9)])
        ...
        for i in range(9):
            if (totalkeys[i]>0):
                ...
                if (i==1):
                    key_list.append("MD_spin")
                if (i==2):
                    key_list.append("MC_atom")
                if (i==3):
                    key_list.append("MC_spin")
                if (i==4):
    ...
    for key in key_list:
        ...
        exploration_movement_args['n_spin_steps_n_calls'] = 0
        ...
        if (key=="MD_atom" or key=="MC_atom"):
            ...
            exploration_movement_args['n_spin_steps_n_calls'] = 1
            # one call to do_atom_walk per walk_single_walker call
            exploration_movement_args['n_spin_steps_per_call'] = 1
            # do_atom_walk makes one do_MC_walk/do_MD_walk per call
            ...
            exploration_movement_args['n_spin_steps_per_call'] = 1
            # do_atom_walk makes one do_MC_walk/do_MD_walk per call
            exploration_movement_args['spin_traj_len'] = 1
...
def do_ns_loop():
    ...
    #tracemalloc.start() #GEORGE

    #snapshot_last = tracemalloc.take_snapshot() #GEORGE

    #snapshot_init = tracemalloc.take_snapshot() #GEORGE

    i_ns_step = start_first_iter
    while ns_args['n_iter'] < 0 or i_ns_step < ns_args['n_iter']:
        #PRINT INFO[NS_ENERGY] & MAX_ENERGY
        ...
        if ns_args['debug'] >= 20:
            ...
            print( print_prefix, "%30s" % ": LOOP_ns_energy START 00 ", i_ns_step, ["%.10f" % at.info['ns_energy'] for at in walkers] )
            for at in walkers:
                if (abs(eval_energy(at)-at.info['ns_energy']) > 10e-8):
        ...
        if output_this_iter:
                ...
                #snapshot_current = tracemalloc.take_snapshot() #GEORGE
                #stats_diff = snapshot_current.compare_to(snapshot_last, 'lineno')
                #stats_abs = snapshot_current.statistics('lineno')
                #del(snapshot_current)
                #del(snapshot_last)
        ...
        for i_at in clone_walk_ind:
            #print("GEORGE CLONE, STEP: ", i_at, i_ns_step)
            if ns_args['debug'] >= 4:
                print( print_prefix, "INFO: 40 WALK clone_target ", rank, i_at)
            #i=0
            #for at in walkers:
                #print(f"GEORGE // WALKER {i} // BEFORE WALK // STEP {i_ns_step} // CLONE {i_at} // Energy =", eval_energy_PE(at))
            #    print(at.arrays)
            #    print(at.info)
            #    i += 1
            #pot.lmp.command(f"#GEORGE // WALKER {i_at} // BEFORE WALK // STEP {i_ns_step} // CLONE {i_at}")
            walk_stats = walk_single_walker(walkers[i_at], movement_args, Emax_of_step, KEmax)
            #pot.lmp.command(f"#GEORGE // WALKER {i_at} // AFTER WALK // STEP {i_ns_step} // CLONE {i_at}")
            #i=0
            #for at in walkers:
                #print(f"GEORGE // WALKER {i} // AFTER WALK // STEP {i_ns_step} // CLONE {i_at} // Energy =", eval_energy_PE(at))
            #    print(at.arrays)
            #    print(at.info)
            #    i += 1
            ...
            #print(print_prefix, "WALK on rank ", rank, "at iteration ", i_ns_step, " walker ", i_at )
            if ns_args['debug'] >= 10 and size <= 1:
            ...
            if rank == 0:
                ...
                for e in initial_unchanged:
                    if e not in final_PE:
                        ...
                        print( "final_PE "+("{:25.19f}"*len(final_PE)).format(*final_PE))
    ...
    #snapshot_final = tracemalloc.take_snapshot() #GEORGE

    #final_compare = snapshot_final.compare_to(snapshot_init, 'lineno') #GEORGE

    #final_stats = snapshot_final.statistics('lineno') #GEORGE

    #print(" [ Final Top 50 results ] ")

    #for stat in final_stats[:50]:
    #    print(stat)

    #print(" [ Final Top 50 differences ] ")

    #for stat in final_compare[:50]:
    #    print(stat)
...
#DOC ``main``: parse arguments
        ...
        ns_args['random_initialise_moms'] = str_to_logical(args.pop('random_initialise_moms', "T"))
        ns_args['mag_norm_init_zero'] = str_to_logical(args.pop('mag_norm_init_zero', "F"))
        ...
        if ns_args['energy_calculator'] == 'quip':
            ...
            try:
                from lammpslib_test import LAMMPSlib
            except:
        ...
        movement_args['n_spin_steps'] = int(args.pop('n_spin_steps', 1))
        movement_args['spin_traj_len'] = int(args.pop('spin_traj_len', 8))
        movement_args['spin_traj_len_cost_multiplier'] = int(args.pop('spin_traj_len_cost_multiplier', 1))
        movement_args['break_up_spin_traj'] = str_to_logical(args.pop('break_up_spin_traj', "T"))
        if movement_args['n_spin_steps'] == 0:
            movement_args['n_spin_steps_per_call'] = 0
            movement_args['n_spin_steps_n_calls'] = 0
        else:
            if movement_args['break_up_spin_traj']:
                movement_args['n_spin_steps_per_call'] = 1
                movement_args['n_spin_steps_n_calls'] = movement_args['n_spin_steps']
            else:
                movement_args['n_spin_steps_per_call'] = movement_args['n_spin_steps']
                movement_args['n_spin_steps_n_calls'] = 1
        ...
        if (movement_args['n_atom_steps'] <= 0 and
            movement_args['n_spin_steps'] <= 0 and
        ...
        try:
            movement_args['spin_algorithm'] = args.pop('spin_algorithm')
        except:
            exit_error("Failed to read algorithm for spin motion spin_algorithm", 1)
        if movement_args['spin_algorithm'] != 'MC' and movement_args['spin_algorithm'] != 'MD':
            exit_error("Got unknown spin_algorithm '%s'\n" % movement_args['spin_algorithm'], 3)
        ...
        if movement_args['fixed_positions']:
            movement_args['do_velocities'] = False
        ...
        movement_args['MD_spin_timestep'] = float(args.pop('MD_spin_timestep', 0.1))
        movement_args['MD_spin_timestep_max'] = float(args.pop('MD_spin_timestep_max', 2.0))
        movement_args['MD_spin_energy_fuzz'] = float(args.pop('MD_spin_energy_fuzz', 1.0e-2))
        movement_args['MD_spin_reject_energy_violation'] = str_to_logical(args.pop('MD_spin_reject_energy_violation', "F"))
...
#           movement_args['adjust_step_interval'] = 20
        ...
        if ns_args['restart_file'] == '': # start from scratch
            ...
            if ns_args['start_config_file'] is not None:
                if movement_args['n_cell_volume_steps'] > 0:
                    for at in walkers:
                        break
                        (p_accept, transform) = propose_volume_step(at, movement_args['MC_cell_volume_per_atom_step_size']/100, movement_args['MC_cell_flat_V_prior'])
                        orig_cell = at.get_cell()
                        new_cell = np.dot(orig_cell,transform)
                        at.set_cell(new_cell, scale_atoms=True)
            ...
            if movement_args['fixed_moments']:
                rng.switch_to_common()
                moments_fixed = rng.float_uniform(-1.0, 1.0, (len(at), 3))
                rng.switch_to_local()
                i = 0
                for moment in moments_fixed:
                    m= 3.4 # placeholder
                    moment_size = m # * (1 + rng.float_uniform(-0.1, 0.1))
                    if movement_args['set_moments'] == 'fm':
                        moment[0] = 0
                        moment[1] = 0
                        moment[2] = 1
                    elif movement_args['set_moments'] == 'afm':
                        moment[0] = 0
                        moment[1] = 0
                        if i%2 == 0:
                            moment[2] = 1
                        ...
                        pass
                    moments_fixed[i] = moment_size * moment/(moment[0]**2+moment[1]**2+moment[2]**2)**(1/2.) 
                    i += 1
                    ...
                    if ns_args['random_initialise_moms']:
                        ...
                        while eval_mag_norm(at) > 0.05 and n_try < ns_args['random_init_max_n_tries']:
                            ...
                            for moment in moments:
                                m= 2.2 # placeholder
                                moment_size = m # * (1 + rng.float_uniform(-0.1, 0.1))
                                moments[i] = moment_size * moment / np.linalg.norm(moments[i])
                                i += 1
                            at.set_initial_magnetic_moments( moments )
                            n_try += 1
                        i = 0
                        if ns_args['mag_norm_init_zero']:
                            i = 0
                            for i in range(int(len(at)/2)):
                                moments[i*2] = -moments[i*2+1]
                                i += 1
                            if len(at)%2 == 1:
                                n_try = 0
                                while eval_mag_norm(at) > 0.001 and n_try < ns_args['random_init_max_n_tries']:
                                    moments[-1] = rng.normal(1, (1, 3))
                                    at.set_initial_magnetic_moments( moments )
                                    n_try += 1

                            moments = moments + rng.normal(1e-6, (len(at), 3))
                        moments /= np.linalg.norm(moments, axis=1, keepdims=True) / 2.2
                        np.random.shuffle(moments)
                        at.set_initial_magnetic_moments( moments )
                    if movement_args['2D']:
                        moments[:,2] = 0
                        if movement_args['potts_param'] > 0:
        ...
        if rank == 0:
            pass
            #snapshot_final = tracemalloc.take_snapshot() #GEORGE
            #final_stats = snapshot_final.compare_to(snapshot_init, 'lineno')
