...
End LAMMPSlib Interface Documentation
    ...
    """
    from memory_profiler import profile #GEORGE

    implemented_properties = ['energy', 'forces', 'stress', 'magmoms', 'initial_magmoms', 'torques']
...
#        self.lmp.put_coosrds(lmp_c_positions)
    ...
    def set_lammps_moms(self, atoms):

        moments = atoms.get_initial_magnetic_moments() #np.array(atoms.arrays['initial_magmoms'])

        #lmp_moments = list(moments.ravel())
        #lmp_c_moments =\
        #    (ctypes.c_double * len(lmp_moments))(*lmp_moments)
        #self.lmp.scatter_atoms('mu', 1, 3, lmp_c_moments)
        ...
        for i in (range(n)):
            ...
            s = np.linalg.norm(moments[i])
            
            sx *= 1/s
            sy *= 1/s
            sz *= 1/s

            self.lmp.command(f"set atom {j} spin  {s} {sx} {sy} {sz}") 

    def calculate(self, atoms, properties, system_changes):
        self.propagate(atoms, properties, system_changes, 0)
    
    def propagate(self, atoms, properties, system_changes, n_steps, dt=None, dt_not_real_time=False, velocity_field=None, magnetic_field=None, debug=False):
        ...
        if not self.initialized:
            if magnetic:
                self.lmp.command("# INITIALISE (MAGNETIC)")
                self.initialise_lammps(atoms, magnetic_field=magnetic_field)
            else:
                self.lmp.command("# INITIALISE (ATOMIC)")
            ...
            if magnetic:
                if n_steps == 0:
                    self.lmp.command("# CALCULATE (MAGNETIC)")
                else:
                    self.lmp.command("# PROPAGATE (MAGNETIC)")
                    self.lmp.command("set atom * spin  1 0 0 1") #reset spins: probably not necessary but wanted to keep consistent with atoms.
            else:
                if n_steps == 0:
                    self.lmp.command("# CALCULATE (ATOMIC)")
                else:
                    self.lmp.command("# PROPAGATE (ATOMIC)")
        ...
        if magnetic:
            if debug:
                if n_steps > 0:
                    t_pre_set_moms = time.time()
            self.set_lammps_moms(atoms)
            if debug:
                if n_steps > 0:
                    t_post_set_moms = time.time()
                    t_set_moms = t_post_set_moms - t_pre_set_moms
                    print("##### PROPAGATE TIMES #####")
                    print(f"SET-MOMS-TIME {t_set_moms}")
                pass
                #print("######### LAMMPSLIB START #########")
                #print("Getting magnetic moments")
            #init_magmoms = atoms.arrays['initial_magmoms']
            if debug:
                pass
                #print(init_magmoms)
                #print("Setting magnetic moments")
            #self.results['initial_magmoms'] = init_magmoms.copy()
            if debug:    
                pass
                #print('results[initial_magmoms] =')
                #print(self.results['initial_magmoms'])
...
#            self.lmp.put_coosrds(lmp_c_velocities)
        ...
        if magnetic:
            pass
            #self.lmp.command('fix freeze all setforce/spin NULL NULL NULL')

        if debug:
            if n_steps > 0:
                pass
                t_pre_run = time.time()
                #t_pre_run = t_pre_extract - t_start
                print("PRE-RUN-TIME", t_pre_run-t_start)
                #print("SETMOMS-TIME", t_pre_run - t_pre_set_moms)
        ...
        if debug:
            if n_steps > 0:
                ...
                #t_pre_run = t_pre_extract - t_start
                print("RUN-TIME", t_post_run-t_pre_run)
                #print("SETMOMS-TIME", t_pre_extract - t_pre_set_moms)

        if magnetic:
            outsp = self.lmp.numpy.extract_compute("outsp", 1, 2)
            magmoms = np.array(outsp[:, :3])
            for i in range(len(magmoms)):
                magmoms[i] *= outsp[i, 3]
            #self.results['magmoms'] = magmoms.copy()
        ...
        if magnetic:
            try:
                ...
                #    magmoms[i] *= outsp[i, 3]
                #if debug:
                #    print("POST-WALK MOMENTS")
                #self.results['magmoms'] = magmoms #.copy()
                #atoms.arrays['magmoms'] = magmoms.copy()
                #atoms.moments = magmoms
                if debug:
                    pass
                    #print("atoms.moments")
                    #print(atoms.moments)#self.results['magmoms'])
                    #print("self.results['magmoms']")
                    #print(self.results['magmoms'])
                    #print("atoms.arrays['magmoms']")
                    #print(atoms.arrays['magmoms'])
                    #print("MOMENT WALK DISPLACEMENT")
                    #print(self.results['magmoms'] - self.results['initial_magmoms'])
                if n_steps > 0:
                    if 'magmoms' in system_changes:
                        pass
                        #atoms.set_initial_magnetic_moments(magmoms)#self.results['magmoms'])
                        
            except:
                print("LAMMPS has been initialised without magnetism enabled, 'magmoms' cannot be calculated.")
                print("'magmoms' not found")
                return
        ...
        if 'torques' in properties:
            try:
                torques = np.array(outsp[:, 4:7]) 
                self.results['torques'] = torques.copy()
            except:
                print("'torques' cannot be calculated without 'moments' in properties or system_changes.")

        if ('magmoms' in properties) or ('magnorm' in properties) or ('E_mag' in properties) or ('T_mag' in properties):
            outmag = self.lmp.numpy.extract_compute("out_mag", 0, 1)

        if 'magnorm' in properties: #average magnetisation
            magnorm = outmag[3]
            self.results['magnorm'] = magnorm

        if 'E_mag' in properties: #magnetic energy
            E_mag = outmag[4]
            self.results['E_mag'] = E_mag

        if 'T_mag' in properties: #magnetic "temperature"
            T_mag = outmag[5]
            self.results['T_mag'] = T_mag
        if debug:
            pass
            #print("######### LAMMPSLIB END #########")
        ...
        if magnetic:
            pass
            #self.lmp.command("uncompute out_mag")
            #self.lmp.command("uncompute outsp")
            #try:
            #    del(outmag)
            #except:
            #    pass
        ...
        if debug:
            if n_steps > 0:
                pass
                t_end = time.time()
                print("POST-RUN-TIME", t_end-t_post_run)
    ...
    def initialise_lammps(self, atoms, magnetic_field=None):
        ...
        if magnetism: # compute and print magnetic properties
            pass
            #self.lmp.command('fix freeze all setforce NULL NULL NULL')
            #if magnetic_field is not None:
            #        self.lmp.command(f"fix B_field all precession/spin zeeman {magnetic_field}")
            self.lmp.command('compute outsp all property/atom spx spy spz sp fmx fmy fmz')
            self.lmp.command('compute out_mag all spin')
            self.lmp.command('thermo_style custom pe ke emol ecoul c_out_mag[5] etotal pxx c_out_mag[*]')
        ...
        #if magnetism:
        #    self.set_lammps_moms(atoms) #GEORGE
